...
pip install librosa pretty_midi mido soundfile basic-pitch crepe mlx-whisper
deactivate

# (2) Demucs venv  (Python 3.11)
python3.11 -m venv .venv-demucs && source .venv-demucs/bin/activate
pip install --upgrade pip wheel
pip install demucs==4.0.1        # first run downloads htdemucs_ft (~500 MB)
deactivate
```

Notes:
- Not on Apple Silicon: use `openai-whisper` instead of `mlx-whisper`.
- On Apple Silicon, `mlx-demucs` runs the same model ~9× faster than torch-CPU demucs.
